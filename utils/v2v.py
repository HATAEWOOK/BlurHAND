...
class V2FModel(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.output_layer = nn.Conv3d(32, 21, kernel_size=1, stride=1, padding=0)
        self.pool3d = nn.AvgPool3d(2)

        self.fc1 = nn.Linear(32**3*32, output_channels)

        self._initialize_weights()

    def forward(self, x):
        x = self.front_layers(x)
        x, encoded = self.encoder_decoder(x)
        x = self.back_layers(x)
        heatmap_3d = self.output_layer(x)
        encoded = encoded.view(encoded.shape[0], -1)
        return encoded, heatmap_3d
